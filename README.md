...
```
.
├─build : Quasarのビルドスクリプト（webpackの設定ファイルもここにあります）
├─config : Quasarの環境設定
├─template : Quasarでページを自動生成する際のテンプレート（Typescriptで使う場合は要修正）
├─src : フロントエンドのソースのroot
│  ├─assets : 静的ファイル置き場（jsから相対パスで取得可能）
│  ├─components : 画面コンポーネント群をここに配置
│  ├─model : ViewやAPIのパラメータをモデルとしてここに配置
│  ├─network : httpのクライアントの置き場でひとまず配置
│  ├─statics : 静的ファイル置き場（絶対パスで取得可能、assetとの使い分けが不明...）
│  ├─store : vuexの状態管理クラスを配置
|  | └─modules : モジュールごとにファイルを分割してここに配置する
│  ├─themes : iosやandroid用のテーマファイル
│  ├─main.ts : アプリケーションのBootstrapとなるファイル
│  └─router.ts : 各画面のコンポーネント（Vueファイル）とURLパスのマッピング
└─node_modules
```
